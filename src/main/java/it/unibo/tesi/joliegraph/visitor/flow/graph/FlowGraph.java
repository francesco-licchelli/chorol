...
public class FlowGraph extends DefaultDirectedGraph<State, RequestEdge> {
	...
	public FlowGraph(String serviceName, boolean isInInput, Operation op, TypeHolder typeHolder, FlowGraph process) {
		super(RequestEdge.class);
		State start = createState();
		State end = createState();
		String opType = isInInput ? "INPUT" : "OUTPUT";
		...
		if (op instanceof OneWayOperation)
			...
			this.addEdge(link.getEndNode(), end).setLabel(
					serviceName,
					op.getName(),
					((ReqResOperation) op).getResponseType(),
					opType + " RESPONSE"
			);
		}
	}
